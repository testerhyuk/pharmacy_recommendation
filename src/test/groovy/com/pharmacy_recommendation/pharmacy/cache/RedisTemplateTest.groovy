package com.pharmacy_recommendation.pharmacy.cache
...
class RedisTemplateTest extends AbstractIntegrationContainerBaseTest{
    ...
    def "RedisTemplate set operations"() {
        given:
        def setOpeations = redisTemplate.opsForSet()
        def key = "setKey"

        when:
        setOpeations.add(key, "h", "e", "l", "l", "o")

        then:
        def size = setOpeations.size(key)
        size == 4
    }
